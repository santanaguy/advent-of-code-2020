...
void Main()
{
	int aggregate(Dictionary<string, List<(int,string)>> inp, string search) =>
		!inp.ContainsKey(search) ? 0 : inp[search].Sum(x => x.Item1 + x.Item1 * aggregate(inp, x.Item2));
	
	var dict = (from match in Regex.Matches(input, @"(\w* \w*) bags contain (\d* \w* \w*) bag(?:s)?(?:, (\d* \w* \w*) bag(?:s)?)*").Cast<Match>()
	from grp in match.Groups.Cast<Group>().Skip(2).Where(x=> x.Success)
	from capture in grp.Captures.Cast<Capture>()
	let count = int.Parse(Regex.Match(capture.Value,@"(\d*)").Groups[0].Value)
	let content = (count, new string(capture.Value.SkipWhile(char.IsNumber).ToArray()).Trim())
	group content by match.Groups[1].Value)
	.ToDictionary(x=> x.Key, y=> y.ToList());
	
	aggregate(dict, "shiny gold").Dump();
}

	string input = @"posh blue bags contain 5 plaid chartreuse bags, 3 plaid lime bags.
