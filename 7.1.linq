...
void Main()
{
	IEnumerable<string> dfs(Dictionary<string, List<string>> inp, string search) =>
		!inp.ContainsKey(search) ? new string[0] : inp[search].SelectMany(x => dfs(inp, x).Append(x));
	...
	group match.Groups[1].Value by capture.Value)
	.ToDictionary(x=> x.Key, y=> y.ToList());
	
	dfs(dict, "shiny gold").Distinct().Count().Dump();
}
