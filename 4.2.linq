...
void Main()
{
	Func<Match,int,int,bool> validNumber = (match,min,max) => int.Parse(match.Groups[1].Value) >=min && int.Parse(match.Groups[1].Value) <=max;
	Func<Match,bool> p = (m) => m.Groups[2].Value == "cm" ?validNumber(m,150,193):validNumber(m,59, 76);

	var patterns = new (Regex, Func<Match,bool>)[] {
		(new Regex(@"byr:(\d{4})\s"), (Match x) => validNumber(x,1920,2002)),
		(new Regex(@"iyr:(\d{4})\s"), (Match x) => validNumber(x,2010,2020)),
		(new Regex(@"eyr:(\d{4})\s"), (Match x) => validNumber(x,2020,2030)),
		(new Regex(@"hgt:(\d\d\d?)(cm|in)\s"), p),
		(new Regex(@"hcl:#([0-9a-f]{6})\s"), (Match m) => true),
		(new Regex(@"ecl:(amb|blu|brn|gry|grn|hzl|oth)\s"), (Match m) => true),
		(new Regex(@"pid:([0-9]{9})\s"),(Match m) => true)
	};

	Regex.Split(text, @"^\s*$", RegexOptions.Multiline)
	.Select(x => patterns.Count(y => y.Item1.Match(x).Success && y.Item2(y.Item1.Match(x))))
	.Count(x => x == 7)
	.Dump();
}

string text = @"byr:1971
iyr:2017 hgt:160cm
eyr:2020 ecl:hzl
pid:157096267
